...
//IMPORTANT. Notice i removed the typedef for the SortedList pointer as well
//as the iterator pointers. It literally is a purely semantic change
//as the exact same datatype will be returned.

//prototypes for helper functions
void  addHead (void* val, SortedList* list);
int ListLength (listnode* list);
listnode* SelectPivot(listnode* list);
 ...
 * 
 * SLCreate must return NULL if it does not succeed, and a non-NULL SortedList*
 *   on success.
 */
 ...
 *
 * SLCreateIterator should return a non-NULL SortedListIterator* on success.
 * SLCreateIterator should return a NULL SortedListIterator* if it could not
 *  construct a SortedListIterator, or if the SortedList* parameter 'list' is NULL.
 */
 ...
 *
 * SLDestroyIterator should destroy the SortedList, but should NOT
